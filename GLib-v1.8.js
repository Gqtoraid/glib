...
/**
  ...
  *   - Add Enum Class type, create typescript like enums. Example: "
		let Blue, Red;
  		let m = new Enum({ Blue, Red }); // Output is Enum { Blue: 0, Red: 1 }
  	  "
  *   - Add JSON [forEach(key, value)]
  *   - Add True & False variables that point to the javascript versions of true & false. (For Python Users that also use JS & are gonna use my library)
  *	  - Add getDayTime function to get wether its night, afternoon, or morning + the current date/time.
  *
...
const True = true;
const False = false; // For People use to Python & are wanting to use my library!
...
function getDaytime(date) {
    return new Promise(async (a, b) => {
        let rtn = { date: null, isMorning: null, isAfternoon: null, isNight: null };
        let isSizeNone = Object.keys(rtn).length <= 0;
        setInterval(() => {
            let realDate = date || new Date(Date.now());
            rtn.date = realDate;
            rtn.isNight = realDate.getHours() > 16;
            rtn.isAfternoon = realDate.getHours() > 12;
            rtn.isMorning = realDate.getHours() < 12;
        });
        if (!rtn || isSizeNone)
        	b({ message: "Failed.", data: rtn });
        a({ message: "Success!", data: rtn });
    });
}
...
class Enum {
	constructor(o = {}) {
		this.o = o;
		Object.keys(this.o).forEach((key, i) => {
			this.o[key] = i;
			this.o[key.toUpperCase()] = this.o[key];
			delete this.o[key];
		});
		Object.freeze(this.o);
		return this.o;
	}
}